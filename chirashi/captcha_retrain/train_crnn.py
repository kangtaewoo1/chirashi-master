...
학습 후 ONNX로 내보내 추론에 사용(kcaptcha_model.onnx). torch(CPU)만 사용.
"""
...
class CRNN(nn.Module):
    def __init__(self):
        ...
        self.cnn = nn.Sequential(
            ...
            blk(128, 256), blk(256, 256), nn.MaxPool2d((2, 1)),  # 4x40
            blk(256, 256), nn.MaxPool2d((2, 1)),                 # 2x40
        )
        self.rnn = nn.LSTM(256 * 2, 256, num_layers=2, bidirectional=True, batch_first=True)
...
def main():
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        if acc >= best:
            best = acc
            torch.save(net.state_dict(), os.path.join(HERE, 'kcaptcha_crnn.pt'))
    print(f'=== 최고 검증 정확도: {best:.1%} → kcaptcha_crnn.pt 저장 ===')

    # ONNX 내보내기(최고 모델 로드)
    net.load_state_dict(torch.load(os.path.join(HERE, 'kcaptcha_crnn.pt')))
    net.eval()
    dummy = torch.zeros(1, 1, IMG_H, IMG_W)
    onnx_path = os.path.join(HERE, 'kcaptcha_model.onnx')
